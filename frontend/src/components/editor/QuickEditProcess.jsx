...
import { getProcessByNavigation, updateProcess } from '../../services/processes';
...
const QuickEditProcess = ({ processId, canEdit }) => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const data = await getProcessByNavigation(processId);
        setProcess(data);
        const body = data?.content_text || '';
        setParagraphs(body.split(/\n\s*\n/).filter(p => p.trim().length > 0));
      } catch (e) {
        setToast({ type: 'error', message: e.response?.data?.error || 'Failed to load process' });
  ...
  const handleParagraphSave = async (index, newText) => {
    ...
    try {
      await updateProcess(process.id, { content_text: newBody });
  ...
  if (loading) return <p className="text-gray-500">Loading process...</p>;

  return (
    <div className="bg-white border rounded-lg p-4 space-y-4">
      {toast && <InlineToast type={toast.type} message={toast.message} onClose={() => setToast(null)} />}
      {paragraphs.length === 0 ? (
        <p className="text-gray-500 italic">No process steps defined yet.</p>
      ) : (
        paragraphs.map((p, i) => (
          <QuickEditParagraph key={i} text={p} canEdit={canEdit} onSave={(newText) => handleParagraphSave(i, newText)} />
        ))
      )}
    </div>
  );
